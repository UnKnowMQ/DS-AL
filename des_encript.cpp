...
class Des_encript{
	...
	string xor_add(string s,string k )
	{
		string add = "";
...
public:
	void encrypt(const string &s, const string &key)
	{
		...
		string R[16];
		 ...
		 L[0] = shift_bit(key_firsthalf,num_leftShift[0]) ;
		 R[0] = shift_bit(key_secondhalf,num_leftShift[0]);
		for(int i = 1;i < 16;i++)
		{
			 L[i] = shift_bit(L[i-1],num_leftShift[i]);
			 R[i] = shift_bit(R[i-1],num_leftShift[i]);
		}
		//to_key_48
		string keys_56[16];
		string keys_48[16];
		...
		for(int i = 0;i < 16;i++)
		{
			 ...
			 for(int j = 0;j < 48;j++)
			 {
			 	keys_48[i] = keys_48[i] + keys_56[pc_2[i]-1];
			 }
			 
		}
				// working on the plain text
		string plain_txt_64 = Hex_to_Bin(s); // transforming key from 16-bits hex to 64-bits bin
		...
		R_xor_K[0] = xor_add(R_48[0], keys_48[0]); // fill the R_xor_K array

		for (int j = 0; j <48; j += 6) // dividing each value of R_xor_K to 8 string contaning 6 char each
			for (int k = j; k < j + 6; k++)
				s1[0][j / 6] += R_xor_K[0][k];

		s_1[0] = "";
		for (int j = 0; j < 8; j++)
			s_1[0] += get_element_from_box(s1[0][j], j);
...

		{
			...
			R_xor_K[i] = xor_add(R_48[i], keys_48[i]); // fill the R_xor_K

			for (int j = 0; j <48; j += 6) // dividing each value of R_xor_K to 8 string contaning 6 char each
				for (int k = j; k < j + 6; k++)
					s1[i][j / 6] += R_xor_K[i][k];

			s_1[i] = "";
			for (int j = 0; j < 8; j++)
				s_1[i] += get_element_from_box(s1[i][j], j);
